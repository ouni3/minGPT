...
class Encoder:
    ...
    * **预分词 (Pre-tokenization):** 在使用像 BPE 这样的子词单元分词算法之前，通常会先对文本进行预分词。
    预分词会将文本分割成更小的单元，例如单词、标点符号等。
    * **BPE 合并:** BPE 算法会迭代地将出现频率最高的字符对合并成一个新的子词单元。
    这个过程会持续进行，直到达到预设的词汇表大小或满足其他停止条件。
    * **问题:** 如果预分词阶段没有考虑到大小写敏感的情况，那么 BPE 算法可能会将同一个词的不同大小写形式视为不同的单元。
    例如，"The" 和 "the" 可能会被视为两个不同的词，即使它们在语义上是相同的。

    **注释解释**

    * **`re.IGNORECASE`:**  这是一个正则表达式标志，表示在进行正则表达式匹配时忽略大小写。
    * **作用:**  这段注释建议在预分词阶段使用 `re.IGNORECASE` 标志，以便将文本转换为小写形式。
    这样做可以确保 BPE 算法将同一个词的不同大小写形式视为相同的单元，从而提高分词的一致性和效率。

    **示例**

    假设我们有一个包含缩写词 "Mr." 的句子："Mr. Smith went to the store."

    * **不使用 `re.IGNORECASE`:** 预分词可能会将句子分割为 ["Mr.", "Smith", "went", "to", "the", "store", "."]。
    由于 "Mr." 中包含大写字母，BPE 算法可能会将其视为一个独立的单元，而不会将其与其他单词中的 "mr" 合并。
    * **使用 `re.IGNORECASE`:** 预分词会将句子转换为小写形式，并将其分割为 ["mr.", "smith", "went", "to", "the", "store", "."]。
    这样，BPE 算法就可以将 "mr." 与其他单词中的 "mr" 合并，从而生成更一致和有效的子词单元。
